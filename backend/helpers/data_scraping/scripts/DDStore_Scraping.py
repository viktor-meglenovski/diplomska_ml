...
def scrape_data(folder_path):
    ...
    load_products(stats, all_products,
                  'https://ddstore.mk/computersandlaptops/notebooksandaccessories/notebooks.html?p={i}&product_list_limit=48',
                  'LAPTOP')
    load_products(stats, all_products,
                  'https://ddstore.mk/computersandlaptops/SmartphonesandAccessories/smartphones.html?p={i}&product_list_limit=48',
                  'PHONE', price_lower_bound_threshold=3000)
    load_products(stats, all_products,
                  'https://ddstore.mk/MonitorsTVandProjectors/TelevisionsandEquipment/televisions.html?p={i}&product_list_limit=48',
                  'TV')
    load_products(stats, all_products,
                  'https://ddstore.mk/computercomponents/graphiccards.html?p={i}&product_list_limit=48',
                  'GPU', price_lower_bound_threshold=2000)
    load_products(stats, all_products,
                  'https://ddstore.mk/computercomponents/processors.html?p={i}&product_list_limit=48',
                  'CPU')
    ...
    df['store'] = 'DDStore'
    df['date'] = date.today()
    file_name = f"{folder_path}\\ddstore_{date.today()}.csv"
